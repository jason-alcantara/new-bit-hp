 ...
 * @return array
 */
function grafique_lite_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! grafique_lite_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'grafique_lite_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'grafique_lite_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'grafique_lite' ) . $network['name'],
 ...
 * @return string
 */
function grafique_lite_get_single_share_buttons() {
	ob_start();
